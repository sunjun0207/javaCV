...
//	private AVCodec out_audioCodec;
	 ...
	 * @throws Exception
	 */
	public TestRecord start(String filename, String format, int imageWidth, int imageHeight, int frameRate, int gopSize)
			throws Exception {
		this.format = format;
...
//		 }
	 ...
	 * @throws Exception
	 */
	private void processVideoConfig(String format_name) throws Exception {
...
//			out_audioStream.time_base().num(1).den(sampleRate);
	 ...
	 * @return
	 */
	public TestRecord from(String url) {
		in_fc = openInput(url);
		if (in_fc != null) {
			// 解决rtmp检索时间过长问题
			// 限制最大读取缓存
...
//	}
	...
	public TestRecord grabtoPush() throws Exception {
		AVPacket pkt = new AVPacket();
	
		for (int errIndex = 0; av_read_frame(in_fc, pkt) >= 0;) {
	...
	public static void main(String[] args) throws Exception {
		TestRecord recorder = new TestRecord().from("rtmp://media3.sinovision.net:1935/live/livestream")
				.start("rtmp://106.14.182.20:1935/rtmp/sunjun", "flv", 800, 600, 25, 2).grabtoPush();
