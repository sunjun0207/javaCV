    ...
    public void stop() throws Exception {
        if (oc != null) {
            try {
                ...
                while (video_st != null && ifmt_ctx == null &&
                        recordImage(0, 0, 0, 0, 0, AV_PIX_FMT_NONE, (Buffer[])null)
                );
                while (audio_st != null && ifmt_ctx == null &&
                        recordSamples(0, 0, (Buffer[])null)
                );
    ...
    public boolean recordImage(int width, int height, int depth, int channels, int stride, int pixelFormat,
        ...
        if (image == null || image.length == 0) {
            ...
            if (video_c.pix_fmt() != pixelFormat || video_c.width() != width || video_c.height() != height) {
                ...
                av_image_fill_arrays(new PointerPointer(picture), picture.linesize(), picture_buf, video_c.pix_fmt(),
                        video_c.width(), video_c.height(), 1);
...
//        }
    ...
    public boolean recordSamples(int sampleRate, int audioChannels, Buffer ... samples) throws Exception {
        ...
        while (true) {
            int inputCount = (int) Math.min(samples != null ? (samples_in[0].limit() - samples_in[0].position()) / (inputChannels * inputDepth)
                    : 0, Integer.MAX_VALUE);
