...
import cc.sunjun.cv.corelib.videoSnapshot.exception.CodecNotFoundExpception;
import cc.sunjun.cv.corelib.videoSnapshot.exception.FileNotOpenException;
import cc.sunjun.cv.corelib.videoSnapshot.exception.StreamInfoNotFoundException;
import cc.sunjun.cv.corelib.videoSnapshot.exception.StreamNotFoundException;
...
public class TestFFmpeg {
	 ...
	 * @throws IOException
	 */
	private static ByteBuffer saveFrame(AVFrame pFrame, int width, int height) {
...
//		IntPointer got_picture = new IntPointer(got_picture_arr);
	...
	public static ByteBuffer openvideo(String url) {
		if(url == null) {
...
//					frame2BufferImage(pFrameRGB, pCodecCtx.width(), pCodecCtx.height());
	...
	public static boolean YV12ToBGR24_FFmpeg(BytePointer pYUV, BytePointer pBGR24, int width, int height)
	{
	    ...
	    imgCtx = sws_getContext(width, height, AV_PIX_FMT_YUV420P, width, height, AV_PIX_FMT_BGR24, SWS_BILINEAR,
